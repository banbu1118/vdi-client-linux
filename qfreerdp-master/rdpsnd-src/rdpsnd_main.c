...
static void rdpsnd_select_supported_audio_formats(rdpsndPlugin* rdpsnd)
{
	...
	{
		WLog_INFO(TAG, "Server announced 0 audio formats, no audio will be played");
		return;
	}

	WLog_INFO(TAG, "Server announced %u audio formats", rdpsnd->NumberOfServerFormats);

	rdpsnd->ClientFormats = audio_formats_new(rdpsnd->NumberOfServerFormats);

	if (!rdpsnd->ClientFormats || !rdpsnd->device)
	{
		WLog_INFO(TAG, "Failed to allocate client formats or no device");
		return;
	}
	...
	for (UINT16 index = 0; index < rdpsnd->NumberOfServerFormats; index++)
	{
		...
		{
			WLog_INFO(TAG, "  format[%u] tag=0x%04x INCOMPATIBLE", index,
			          serverFormat->wFormatTag);
			continue;
		}

		WINPR_ASSERT(rdpsnd->device->FormatSupported);
		BOOL dspSupported = freerdp_dsp_supports_format(serverFormat, FALSE);
		BOOL devSupported = rdpsnd->device->FormatSupported(rdpsnd->device, serverFormat);
		WLog_INFO(TAG, "  format[%u] tag=0x%04x ch=%u rate=%u bits=%u dsp=%d dev=%d", index,
		          serverFormat->wFormatTag, serverFormat->nChannels,
		          serverFormat->nSamplesPerSec, serverFormat->wBitsPerSample,
		          dspSupported, devSupported);
		if (dspSupported || devSupported)
		{
			AUDIO_FORMAT* clientFormat = &rdpsnd->ClientFormats[rdpsnd->NumberOfClientFormats++];
			audio_format_copy(serverFormat, clientFormat);
		}
	}
	WLog_INFO(TAG, "Selected %u audio formats for server", rdpsnd->NumberOfClientFormats);
}
