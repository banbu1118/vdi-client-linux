...
static BOOL rdpsnd_pulse_open_stream(rdpsndDevicePlugin* device)
{
	...
	{
		/* Ensure at least 200ms buffer to prevent underrun/stutter */
		const UINT32 minLatencyMs = 200;
		const UINT32 effectiveLatency =
		    (pulse->latency > 0) ? pulse->latency : minLatencyMs;
